...
impl Pubgmhd for Policy {
    fn pubgmhd(&mut self) -> Result<()> {
        let pid = self.find_pid("com.tencent.tmgp.pubgmhd").context("无法获取应用pid")?;
        let tid = self.find_tid(pid, "Thread-13").context("无法获取进程tid")?;
        Self::set_affinity(tid as libc::pid_t, 6)?;
        Ok(())
    }
}
